...
class BulbapediaSearchInput(BaseModel):
    ...
    max_results: int = Field(default=5, description="Maximum number of results to return. Defaults to 5.")
...
def bulbapedia_ingest_page(args: BulbapediaIngestInput) -> str:
    ...
    return ingest_web_page(IngestWebPageArgs(
        url=args.url,
        css_selector=None,
        use_stealth=False,
        max_age_days=7
...
class BulbapediaQueryInput(BaseModel):
    ...
    filter_url: Optional[str] = Field(default=None, description="Optional exact URL to filter results by.")
...
def bulbapedia_query_content(args: BulbapediaQueryInput) -> str:
    ...
    return query_web_content(QueryWebContentArgs(
        query=args.query,
        n_results=5,
...
- **Step 1:** Use `bulbapedia_search` to find the 1 or 2 most relevant URLs.
...
- **Step 3:** Use `bulbapedia_query_content` to find the exact paragraph answering the user's query from the ingested URL.
...
- The output should be concise, factual, and direct without any conversational padding (Do not say "Here is what I found").
"""
    ...
    def __init__(self, model_name: Optional[str] = None) -> None:
        super().__init__(
            config=AgentConfig(
                ...
                tools=[bulbapedia_search, bulbapedia_ingest_page, bulbapedia_query_content],
                history_limit=25,
            )
        )
