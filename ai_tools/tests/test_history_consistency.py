...
def test_get_consistent_history_backtracks_to_user():
    ...
    hist = agent._get_consistent_history(1)
    assert len(hist) == 3
...
def test_get_consistent_history_skips_forward_if_no_preceding_user():
    ...
    hist = agent._get_consistent_history(2)
    assert len(hist) == 1
    assert hist[0]["role"] == "user"
...
def test_get_consistent_history_empty():
    agent = Agent(system_prompt="sys")
    assert agent._get_consistent_history(5) == []
    assert agent._get_consistent_history(0) == []
    assert agent._get_consistent_history(-1) == []
